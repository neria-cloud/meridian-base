...
import (
	...
	e2eseed "github.com/neria-cloud/meridian-base/tests/cmd/seed"
)
...
func run(ctx context.Context, args []string) error {
	...
	fs.StringVar(&opts.ConfigPath, "config-path", opts.ConfigPath, "optional Meridian config.json path used to derive DB settings")
	fs.StringVar(&opts.EncryptionKey, "encryption-key", opts.EncryptionKey, "optional Meridian encryption key for encrypted config rows")
