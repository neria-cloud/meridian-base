...
func run(cfg fileConfig, count, batch int, prefix string, skipSeed bool, rl rateLimitParams, governanceMode string) (int, error) {
	...
	encrypt.Init(cfg.EncryptionKey, meridian.NewDefaultLogger(schemas.LogLevelWarn))
...
func main() {
	configPath := flag.String("config", "", "path to Meridian config.json (provides encryption_key and postgres DSN)")
