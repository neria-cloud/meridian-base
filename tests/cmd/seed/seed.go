...
func NormalizeOptions(opts Options) (Options, error) {
	...
	if opts.ConfigDSN == "" {
		opts.ConfigDSN = "postgres://meridian:bifrost_password@localhost:5432/meridian?sslmode=disable"
	}
	if opts.LogsDialect == "" {
		opts.LogsDialect = def.LogsDialect
	}
	if opts.LogsDSN == "" {
		opts.LogsDSN = "postgres://meridian:bifrost_password@localhost:5432/meridian?sslmode=disable"
	}
...
func InitEncryption(opts Options) {
	...
	encrypt.Init(opts.EncryptionKey, meridian.NewDefaultLogger(schemas.LogLevelWarn))
}
