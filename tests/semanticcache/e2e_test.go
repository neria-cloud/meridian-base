...
func TestMain(m *testing.M) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[SC-E2E] FATAL: cannot reach Meridian at %s: %v\n", cfg.BifrostURL, err)
		return
	}
	if status != http.StatusOK {
		fmt.Fprintf(os.Stderr, "[SC-E2E] FATAL: GET /api/plugins returned %d (Meridian up at %s?)\n", status, cfg.BifrostURL)
		return
	}
	...
	fmt.Fprintf(os.Stderr, "[SC-E2E] run starting: meridian=%s namespace=%s reports=%s trail_sid=%q\n",
