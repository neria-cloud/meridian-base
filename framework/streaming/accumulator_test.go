	...
	for i := 0; i < 10; i++ {
		chunk := &ChatStreamChunk{
			...
			Delta: &schemas.ChatStreamResponseChoiceDelta{
				Content: meridian.Ptr(fmt.Sprintf("chunk %d", i)),
			},
		}
		if i == 9 {
...
func TestAccumulatedChatStreamPreservesServiceTierBeforeUsageOnlyChunk(t *testing.T) {
	accumulator := NewAccumulator(nil, meridian.NewDefaultLogger(schemas.LogLevelError))
		...
		go func(goroutineID int) {
			...
			for i := 0; i < chunksPerGoroutine; i++ {
				chunk := &ChatStreamChunk{
					...
					Timestamp:  time.Now(),
					Delta: &schemas.ChatStreamResponseChoiceDelta{
						Content: meridian.Ptr(fmt.Sprintf("g%d-c%d", goroutineID, i)),
					},
				}
	...
	priorChunks := []*ResponsesStreamChunk{
		{
			...
			StreamResponse: &schemas.BifrostResponsesStreamResponse{
				...
				Response: &schemas.BifrostResponsesResponse{
					ID: meridian.Ptr("resp_terminal_usage"),
				},
			},
		},
		{
		...
		},
		{
			...
			StreamResponse: &schemas.BifrostResponsesStreamResponse{
				Type:  schemas.ResponsesStreamResponseTypeOutputTextDelta,
				Delta: meridian.Ptr("duplicate index ignored"),
			},
		},
	}
...
func TestProcessStreamingResponseDedupesDuplicateTerminalChunkWithUniqueIndex(t *testing.T) {
	...
	response := &schemas.BifrostResponse{
		ResponsesStreamResponse: &schemas.BifrostResponsesStreamResponse{
			...
			SequenceNumber: 3,
			Response: &schemas.BifrostResponsesResponse{
				ID: meridian.Ptr("resp_monotonic_terminal"),
				Usage: &schemas.ResponsesResponseUsage{
					InputTokens:  11,
					OutputTokens: 7,
					TotalTokens:  18,
...
func newChatChunkResponse(idx int, content string, finishReason *string, usage *schemas.BifrostLLMUsage) *schemas.BifrostResponse {
	delta := &schemas.ChatStreamResponseChoiceDelta{}
	if content != "" {
		delta.Content = meridian.Ptr(content)
	}
