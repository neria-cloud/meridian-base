...
func (a *Accumulator) processImageStreamingResponse(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*ProcessedStreamResponse, error) {
	...
	_, provider, requestedModel, resolvedModel := meridian.GetResponseFields(result, bifrostErr)
	...
	if bifrostErr != nil {
		...
		if result.ImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = meridian.Ptr(fmt.Sprintf("%v", result.ImageGenerationStreamResponse.ExtraFields.RawResponse))
		}
		...
		if isFinalChunk {
			...
			chunk.FinishReason = meridian.Ptr("completed")
		}
	}
