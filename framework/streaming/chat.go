...
func (a *Accumulator) processChatStreamingResponse(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*ProcessedStreamResponse, error) {
	...
	requestType, provider, model, resolvedModel := meridian.GetResponseFields(result, bifrostErr)
	...
	if bifrostErr != nil {
		chunk.FinishReason = meridian.Ptr("error")
		...
		if result.TextCompletionResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = meridian.Ptr(fmt.Sprintf("%v", result.TextCompletionResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
		...
		if result.ChatResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = meridian.Ptr(fmt.Sprintf("%v", result.ChatResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCost(result, modelcatalog.PricingLookupScopesFromContext(ctx, string(result.GetExtraFields().Provider)))
	...
	if isFinalChunk {
		...
		return &ProcessedStreamResponse{
			...
			RoutingInfo:    meridian.GetResponseRoutingInfo(result, bifrostErr),
			Data:           data,
