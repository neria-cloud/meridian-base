...
func TestFilePathFromURL(t *testing.T) {
	cases := []struct {
		...
		{"file:///opt/meridian/pricing.json", "/opt/meridian/pricing.json"},
		{"file://localhost/opt/meridian/pricing.json", "/opt/meridian/pricing.json"},
...
// resolve meridian.neria.cloud.
//
// The fixtures in testdata/ are real entries extracted from the public
// datasheets at https://meridian.neria.cloud/datasheet and
// https://meridian.neria.cloud/datasheet/model-parameters.
func TestLoadFromLocalFiles(t *testing.T) {
	logger := meridian.NewDefaultLogger(schemas.LogLevelWarn)
...
// even when the default meridian.neria.cloud URLs would be unreachable, a file:// URL
