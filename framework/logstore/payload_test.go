...
func TestObjectKey(t *testing.T) {
	...
	key := ObjectKey("meridian", ts, "req_abc123")
	assert.Equal(t, "meridian/logs/2026/04/03/14/req_abc123.json.gz", key)
}

func TestMCPToolObjectKey(t *testing.T) {
	ts := time.Date(2026, 4, 3, 14, 0, 0, 0, time.UTC)
	key := MCPToolObjectKey("meridian", ts, "mcp_abc123")
	assert.Equal(t, "meridian/mcp-logs/2026/04/03/14/mcp_abc123.json.gz", key)
}
