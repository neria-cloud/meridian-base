...
func TestBuildDSNQuotesValuesForPasswordCommandParsing(t *testing.T) {
	...
	require.Equal(t, "meridian", pgxConfig.Database)
}
...
func TestBuildDSNQuotesSpecialCharacters(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(cfg *Config) {
				cfg.DBName = schemas.NewSecretVar(`meridian\tenant's`)
			},
			validate: func(t *testing.T, pgxConfig *pgx.ConnConfig) {
				require.Equal(t, `meridian\tenant's`, pgxConfig.Database)
			},
		},
	}
...
func validConfig() *Config {
	return &Config{
		...
		User:     schemas.NewSecretVar("meridian"),
		Password: schemas.NewSecretVar("password"),
		DBName:   schemas.NewSecretVar("meridian"),
