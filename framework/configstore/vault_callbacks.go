...
func RegisterVaultCallbacks(db *gorm.DB) {
	db.Callback().Create().Before("gorm:before_create").Register("meridian:vault_store", vaultStoreCallback)
	db.Callback().Update().Before("gorm:before_update").Register("meridian:vault_store", vaultStoreCallback)
	db.Callback().Delete().After("gorm:after_delete").Register("meridian:vault_remove", vaultRemoveCallback)
}
