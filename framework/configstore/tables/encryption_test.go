...
func init() {
	encrypt.Init(testEncryptionKey, meridian.NewDefaultLogger(schemas.LogLevelInfo))
}
...
func TestTableOauthToken_EncryptDecrypt(t *testing.T) {
	...
	token := &TableOauthToken{
		...
		TokenType:    "Bearer",
		ExpiresAt:    meridian.Ptr(time.Now().Add(time.Hour)),
	}
...
func disableEncryption(t *testing.T) {
	t.Helper()
	encrypt.Init("", meridian.NewDefaultLogger(schemas.LogLevelInfo))
	t.Cleanup(func() {
		encrypt.Init(testEncryptionKey, meridian.NewDefaultLogger(schemas.LogLevelInfo))
	})
}
