...
func installRoutingRuleDeadlockAmplifier(t *testing.T, db *gorm.DB) {
	t.Helper()
	require.NoError(t, db.Callback().Update().After("gorm:update").Register("meridian:test_sleep_after_routing_rule_update", func(tx *gorm.DB) {
		if tx.Statement != nil && tx.Statement.Table == "routing_rules" {
			_ = tx.Session(&gorm.Session{NewDB: true}).Exec("SELECT pg_sleep(0.05)").Error
		}
	}))
	require.NoError(t, db.Callback().Delete().After("gorm:delete").Register("meridian:test_sleep_after_routing_target_delete", func(tx *gorm.DB) {
