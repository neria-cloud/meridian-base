...
// functions directly.
var testMigrationLogger = meridian.NewDefaultLogger(schemas.LogLevelInfo)
...
const postgresDSN = "host=localhost user=meridian password=bifrost_password dbname=meridian port=5432 sslmode=disable search_path=" + pgTestSchema
...
func setupFullMigrationDB(t *testing.T) (*RDBConfigStore, *gorm.DB) {
	...
	store := &RDBConfigStore{logger: meridian.NewDefaultLogger(schemas.LogLevelInfo)}
	store.db.Store(db)
...
func TestFullMigration_EndToEnd(t *testing.T) {
	...
	for _, vk := range []struct {
		...
		err := store.CreateVirtualKey(ctx, &tables.TableVirtualKey{
			...
			IsActive: meridian.Ptr(true), CreatedAt: now, UpdatedAt: now,
		})
