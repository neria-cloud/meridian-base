...
func TestVaultCallbacks_AutoStoreAndRemove(t *testing.T) {
	...
	headerPath := "meridian/config_mcp_clients/client-1/headers/Authorization"
...
func TestVaultCallbacks_SelfManagedStoresPlaintext(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			encrypt.Init(tc.encryptionKey, meridian.NewDefaultLogger(schemas.LogLevelInfo))
			t.Cleanup(func() { encrypt.Init("", meridian.NewDefaultLogger(schemas.LogLevelInfo)) })
			...
			secretPath := fmt.Sprintf("meridian/config_keys/%s/bedrock_secret_key", keyID)
...
func TestVaultCallbacks_SelfManagedRemovesVaultSecrets(t *testing.T) {
	...
	valuePath := fmt.Sprintf("meridian/config_keys/%s/value", keyID)
