...
import (
	...
	"github.com/neria-cloud/meridian-base/framework/migrator"
	"gorm.io/gorm"
	"gorm.io/gorm/clause"
)
		...
		if time.Now().After(deadline) {
			...
			return nil, fmt.Errorf(
				"failed to acquire configstore migration lock (key=%d) after %d attempts over %s\n\n"+
					"This usually means another Meridian pod (or a previous crashed pod's lingering\n"+
