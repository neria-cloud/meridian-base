...
import (
	...
	"github.com/neria-cloud/meridian-base/core/mcp"
	"github.com/neria-cloud/meridian-base/core/schemas"
	"github.com/neria-cloud/meridian-base/framework/configstore/tables"
	"github.com/neria-cloud/meridian-base/framework/logstore"
	"github.com/neria-cloud/meridian-base/framework/mcpcatalog"
	"github.com/neria-cloud/meridian-base/framework/modelcatalog"
	"github.com/neria-cloud/meridian-base/framework/streaming"
)
...
func applyMCPGovernanceFieldsToEntry(ctx *schemas.BifrostContext, entry *logstore.MCPToolLog) {
	...
	userID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyUserID)
	teamID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceTeamID)
	customerID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceCustomerID)
	businessUnitID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceBusinessUnitID)
	if userID != "" {
		entry.UserID = &userID
	}
	if teamID != "" {
...
func (p *LoggerPlugin) PreLLMHook(ctx *schemas.BifrostContext, req *schemas.BifrostRequest) (*schemas.BifrostRequest, *schemas.LLMPluginShortCircuit, error) {
	...
	if meridian.IsStreamRequestType(req.RequestType) {
		tracer, traceID, err := meridian.GetTracerFromContext(ctx)
	...
	fallbackIndex := meridian.GetIntFromContext(ctx, schemas.BifrostContextKeyFallbackIndex)
...
func (p *LoggerPlugin) PostLLMHook(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	...
	selectedKeyID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedKeyID)
	selectedKeyName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedKeyName)
	virtualKeyID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyID)
	virtualKeyName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyName)
	routingRuleID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceRoutingRuleID)
	routingRuleName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceRoutingRuleName)
	selectedPromptName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedPromptName)
	selectedPromptVersion := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedPromptVersion)
	selectedPromptID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedPromptID)
	teamID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceTeamID)
	teamName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceTeamName)
	customerID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceCustomerID)
	customerName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceCustomerName)
	userID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyUserID)
	userName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyUserName)
	businessUnitID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceBusinessUnitID)
	businessUnitName := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceBusinessUnitName)
	numberOfRetries := meridian.GetIntFromContext(ctx, schemas.BifrostContextKeyNumberOfRetries)
	attemptTrail, _ := ctx.Value(schemas.BifrostContextKeyAttemptTrail).([]schemas.KeyAttemptRecord)

	requestType, _, originalModelRequested, resolvedModelUsed := meridian.GetResponseFields(result, bifrostErr)
	resolvedKeyAlias := meridian.GetResponseRoutingInfo(result, bifrostErr).ResolvedKeyAlias
	shouldStoreRaw, _ := ctx.Value(schemas.BifrostContextKeyShouldStoreRawInLogs).(bool)
	contentLoggingEnabled := p.contentLoggingEnabled(ctx)

	isFinalChunk := meridian.IsFinalChunk(ctx)
	...
	var pendingVal any
	var hasPending bool
	if !meridian.IsStreamRequestType(requestType) || isFinalChunk || bifrostErr != nil {
	...
	if !hasPending {
		...
		if bifrostErr != nil {
			...
			entry := &logstore.Log{
				...
				Stream:    meridian.IsStreamRequestType(requestType),
	...
	var traceID string
	if meridian.IsStreamRequestType(requestType) && requestType != schemas.RealtimeRequest {
		var err error
		tracer, traceID, err = meridian.GetTracerFromContext(ctx)
		if err != nil {
	...
	if meridian.IsStreamRequestType(requestType) && requestType != schemas.RealtimeRequest && !isFinalChunk && result != nil && bifrostErr == nil {
	...
	if requestType == schemas.RealtimeRequest {
		if resolvedRealtimeSessionID := meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyRealtimeSessionID); resolvedRealtimeSessionID != "" {
	...
	if result == nil && bifrostErr != nil {
		...
		applyModelAlias(entry, originalModelRequested, resolvedModelUsed)
		if meridian.IsStreamRequestType(requestType) {
			entry.Stream = true
		}
...
func (p *LoggerPlugin) PreMCPHook(ctx *schemas.BifrostContext, req *schemas.BifrostMCPRequest) (*schemas.BifrostMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	...
	if meridian.IsCodemodeTool(fullToolName) || meridian.IsCodemodeTool(toolName) {
		return req, nil, nil
	}
...
func (p *LoggerPlugin) PostMCPHook(ctx *schemas.BifrostContext, resp *schemas.BifrostMCPResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostMCPResponse, *schemas.BifrostError, error) {
	...
	if resp != nil && meridian.IsCodemodeTool(resp.ExtraFields.ToolName) {
		return resp, bifrostErr, nil
	}
