...
func newTestPlugin(t *testing.T) *PrometheusPlugin {
	t.Helper()
	p, err := Init(&Config{}, nil, meridian.NewDefaultLogger(schemas.LogLevelError))
	if err != nil {
		t.Fatalf("Init: %v", err)
	}
	return p
...
func TestMetricsEnabledGating(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			p, err := Init(&Config{MetricsEnabled: tc.set}, nil, meridian.NewDefaultLogger(schemas.LogLevelError))
			if err != nil {
				t.Fatalf("Init: %v", err)
			}
...
func TestGetMetricsGathererCombinesRegistries(t *testing.T) {
	...
	if !present["bifrost_input_tokens_total"] {
		t.Error("/metrics gatherer missing Meridian metric bifrost_input_tokens_total")
	}
...
func TestPushGatewayLifecycle(t *testing.T) {
	...
	if got.JobName != "meridian" {
		t.Errorf("default JobName = %q, want meridian", got.JobName)
	}
...
func TestPushGatewayPushesBifrostButNotRuntimeCollectors(t *testing.T) {
	...
	case body := <-bodies:
		if !bytes.Contains(body, []byte("bifrost_input_tokens_total")) {
			t.Error("pushed payload missing Meridian metric bifrost_input_tokens_total")
		}
