...
func Init(config *Config, pricingManager *modelcatalog.ModelCatalog, logger schemas.Logger) (*PrometheusPlugin, error) {
	...
	bifrostUpstreamRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of requests forwarded to upstream providers by Meridian.",
		},
	...
	bifrostUpstreamLatencySeconds := factory.NewHistogramVec(
		prometheus.HistogramOpts{
			...
			Help:    "Latency of requests forwarded to upstream providers by Meridian.",
	...
	bifrostSuccessRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of successful requests forwarded to upstream providers by Meridian.",
		},
		append(defaultBifrostLabels, filteredCustomLabels...),
	)
	...
	bifrostErrorRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of error requests forwarded to upstream providers by Meridian.",
		},
	...
	bifrostInputTokensTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of input tokens forwarded to upstream providers by Meridian.",
		},
		append(defaultBifrostLabels, filteredCustomLabels...),
	)
	...
	bifrostOutputTokensTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of output tokens forwarded to upstream providers by Meridian.",
		},
		append(defaultBifrostLabels, filteredCustomLabels...),
	)
	...
	bifrostCacheHitsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of cache hits forwarded to upstream providers by Meridian, separated by cache type (direct/semantic).",
		},
	...
	bifrostMCPToolDuration := factory.NewHistogramVec(
		prometheus.HistogramOpts{
			...
			Help:    "Duration of an MCP tool call as observed by Meridian (the MCP client).",
...
func (p *PrometheusPlugin) PostMCPHook(ctx *schemas.BifrostContext, resp *schemas.BifrostMCPResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostMCPResponse, *schemas.BifrostError, error) {
	...
	if resp != nil {
		...
		clientName = meridian.GetStringFromContext(ctx, mcpClientNameKey)
		toolName = meridian.GetStringFromContext(ctx, mcpToolNameKey)
	}
	if !mcpReqType.IsExecuteTool() || meridian.IsCodemodeTool(toolName) {
		return resp, bifrostErr, nil
	}
	...
	labelValues := map[string]string{
		...
		"virtual_key_id":     meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyID),
		"virtual_key_name":   meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyName),
		"team_id":            meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceTeamID),
		"team_name":          meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceTeamName),
		"customer_id":        meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceCustomerID),
		"customer_name":      meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceCustomerName),
		"business_unit_id":   meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceBusinessUnitID),
		"business_unit_name": meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceBusinessUnitName),
	}
...
func canonicalEntitySet(ctx context.Context, idsKey, namesKey, scalarIDKey, scalarNameKey schemas.BifrostContextKey) (idsCSV, namesCSV string) {
	...
	if len(ids) == 0 {
		if id := meridian.GetStringFromContext(ctx, scalarIDKey); id != "" {
			ids = []string{id}
			names = []string{meridian.GetStringFromContext(ctx, scalarNameKey)}
		}
	}
...
func (p *PrometheusPlugin) PostLLMHook(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	requestType, provider, originalModel, resolvedModel := meridian.GetResponseFields(result, bifrostErr)
	...
	isStreamFinal := !meridian.IsStreamRequestType(requestType) || (hasFinalChunkIndicator && isFinalChunk)
	if isStreamFinal {
		if method, ok := ctx.Value(activeRequestTypeKey).(schemas.RequestType); ok {
			p.ActiveRequests.WithLabelValues(string(method)).Dec()
		}
