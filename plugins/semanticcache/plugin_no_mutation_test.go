...
func TestCachingDoesNotMutateRequestSentToProvider(t *testing.T) {
	...
	client, err := meridian.Init(bctx, schemas.BifrostConfig{
		Account: &BaseAccount{},
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: meridian.Ptr(systemContent),
				},
			},
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: meridian.Ptr(userContent),
				},
			},
		},
		Params: &schemas.ChatParameters{
			Temperature:         meridian.Ptr(0.0),
			MaxCompletionTokens: meridian.Ptr(5),
		},
	}
