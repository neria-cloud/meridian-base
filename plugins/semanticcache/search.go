...
func (plugin *Plugin) generateEmbedding(ctx *schemas.BifrostContext, text string) ([]float32, int, error) {
	...
	meridian.ClearContextForInternalRequest(embeddingCtx)
...
func (plugin *Plugin) buildResponseFromResult(ctx *schemas.BifrostContext, state *cacheState, req *schemas.BifrostRequest, result vectorstore.SearchResult, cacheType CacheType, threshold *float64, inputTokens *int) (*schemas.LLMPluginShortCircuit, error) {
	...
	isStream := meridian.IsStreamRequestType(req.RequestType)
	if isStream {
...
func (plugin *Plugin) stampCacheDebugForHit(
	...
	cd.CacheHit = true
	cd.HitType = meridian.Ptr(string(cacheType))
	cd.CacheID = meridian.Ptr(cacheID)
	cd.RequestedProvider = meridian.Ptr(string(requestedProvider))
	cd.RequestedModel = meridian.Ptr(requestedModel)
	cd.CacheHitLatency = meridian.Ptr(time.Since(state.CreatedAt).Milliseconds())
	if cacheType == CacheTypeSemantic {
		cd.ProviderUsed = meridian.Ptr(string(plugin.config.Provider))
		cd.ModelUsed = meridian.Ptr(plugin.config.EmbeddingModel)
