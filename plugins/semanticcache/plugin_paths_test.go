...
func TestInit_RejectsNilConfig(t *testing.T) {
	if _, err := Init(context.Background(), nil, meridian.NewDefaultLogger(schemas.LogLevelError), newObservableStore()); err == nil {
...
func TestInit_RejectsNilStore(t *testing.T) {
	...
	if _, err := Init(context.Background(), cfg, meridian.NewDefaultLogger(schemas.LogLevelError), nil); err == nil {
...
func TestInit_RejectsNegativeDimension(t *testing.T) {
	...
	if _, err := Init(context.Background(), cfg, meridian.NewDefaultLogger(schemas.LogLevelError), newObservableStore()); err == nil || !strings.Contains(err.Error(), "dimension") {
...
func TestInit_AllowsDirectOnlyMode(t *testing.T) {
	...
	cfg := &Config{Dimension: 1}
	plugin, err := Init(context.Background(), cfg, meridian.NewDefaultLogger(schemas.LogLevelError), newObservableStore())
	if err != nil {
