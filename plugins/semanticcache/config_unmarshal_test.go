...
func TestUnmarshalJSON_BoolPointerFields(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectCacheByModel: meridian.Ptr(true),
			expectCacheByProv:  meridian.Ptr(true),
			expectExcludeSys:   meridian.Ptr(true),
		},
		{
			name:               "all set to false",
			json:               `{"dimension": 1536, "cache_by_model": false, "cache_by_provider": false, "exclude_system_prompt": false}`,
			expectCacheByModel: meridian.Ptr(false),
			expectCacheByProv:  meridian.Ptr(false),
			expectExcludeSys:   meridian.Ptr(false),
		},
		{
			name:               "all omitted",
			json:               `{"dimension": 1536}`,
