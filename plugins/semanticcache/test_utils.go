...
import (
	...
	"github.com/neria-cloud/meridian-base/framework/vectorstore"
	mocker "github.com/neria-cloud/meridian-base/plugins/mocker"
)
...
func getMockRules() []mocker.MockRule {
	return []mocker.MockRule{
		// Core test prompts
		{
			Name:        "meridian-definition",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)What is Meridian.*")},
			Probability: 1.0,
			Responses: []mocker.Response{
				{Type: mocker.ResponseTypeSuccess, Content: &mocker.SuccessResponse{Message: "Meridian is a unified API for interacting with multiple AI providers."}},
			},
		},
		{
			Name:        "machine-learning-explanation",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)what is machine learning\\?|explain machine learning|machine learning concepts|can you explain machine learning|explain the basics of machine learning")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "ai-explanation",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)what is artificial intelligence\\?|can you explain what ai is\\?|define artificial intelligence")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("What is the capital of France\\?")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "newton-laws",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)describe.*newton.*three laws|describe.*three laws.*newton")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Weather-related prompts
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)what.*weather|weather.*like")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Blockchain and deep learning
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)define blockchain|blockchain technology")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "deep-learning",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)what is deep learning")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Quantum computing
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)quantum computing|explain quantum")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Conversation prompts
		{
			Name:        "hello-greeting",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)^hello$|^hi$|hello.*world")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "how-are-you",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)how are you")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)meaning of life")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "short-story",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)tell me.*short story")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Test-specific prompts
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)test configuration")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "test-messages",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)test.*message|test.*no-store|test.*cache|test.*error|ttl test|threshold test|provider.*test|edge case test")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "long-prompt",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)very long prompt")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)test.*parameters|performance test")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Dynamic message patterns (for conversation tests)
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: meridian.Ptr("(?i)message \\d+")},
			Probability: 1.0,
			Responses: []mocker.Response{
...
func getMockedBifrostClient(t *testing.T, ctx *schemas.BifrostContext, logger schemas.Logger, semanticCachePlugin schemas.LLMPlugin, extraPlugins ...schemas.LLMPlugin) *meridian.Bifrost {
	...
	account := &BaseAccount{}
	client, err := meridian.Init(ctx, schemas.BifrostConfig{
		Account:    account,
		LLMPlugins: llmPlugins,
		Logger:     logger,
	})
	if err != nil {
		t.Fatalf("Error initializing Meridian with mocker: %v", err)
	}
...
func NewTestSetupWithVectorStore(t *testing.T, config *Config, storeType vectorstore.VectorStoreType, extraPlugins ...schemas.LLMPlugin) *TestSetup {
	...
	logger := meridian.NewDefaultLogger(schemas.LogLevelDebug)
...
func CreateImageGenerationRequest(prompt string, size string, quality string) *schemas.BifrostImageGenerationRequest {
	return &schemas.BifrostImageGenerationRequest{
		...
		Params: &schemas.ImageGenerationParameters{
			Size:    meridian.Ptr(size),
			Quality: meridian.Ptr(quality),
			N:       meridian.Ptr(1),
		},
	}
}
