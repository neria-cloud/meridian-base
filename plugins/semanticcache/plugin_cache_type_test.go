...
func TestCacheTypeDirectOnly(t *testing.T) {
	...
	testRequest := CreateBasicChatRequest("What is Meridian?", 0.7, 50)
			...
			Input: []schemas.ChatMessage{
				{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: meridian.Ptr(prompt),
					},
				},
			},
		},
...
func TestDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	config.CacheByProvider = meridian.Ptr(false)
	config.CacheByModel = meridian.Ptr(false)
			...
			Choices: []schemas.BifrostResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							...
							Content: &schemas.ChatMessageContent{
								ContentStr: meridian.Ptr("Go schedules lightweight goroutines in user space onto a smaller pool of OS threads."),
							},
						},
					},
				},
...
func TestStreamingDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	chunks := []struct {
		...
		{content: " onto a smaller pool of OS threads.", chunkIndex: 1, finishReason: meridian.Ptr("stop"), streamEnd: true},
	}
				...
				Choices: []schemas.BifrostResponseChoice{
					{
						...
						ChatStreamResponseChoice: &schemas.ChatStreamResponseChoice{
							Delta: &schemas.ChatStreamResponseChoiceDelta{
								Content: meridian.Ptr(chunk.content),
							},
						},
					},
				},
...
func TestCacheTypeDirectUsesChunkLookup(t *testing.T) {
	...
	req := &schemas.BifrostRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: CreateBasicChatRequest("What is Meridian?", 0.7, 50),
	}
