...
func TestJsonParserPluginEndToEnd(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error initializing Meridian: %v", err)
	}
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: meridian.Ptr("Return a JSON object with name, age, and city fields. Example: {\"name\": \"John\", \"age\": 30, \"city\": \"New York\"}"),
				},
			},
		},
	...
	if bifrostErr != nil {
		t.Fatalf("Error in Meridian request: %v", bifrostErr)
	}
...
func TestJsonParserPluginPerRequest(t *testing.T) {
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: meridian.Ptr("Return a JSON object with name and age fields."),
				},
			},
		},
		Params: &schemas.ChatParameters{
	...
	if bifrostErr != nil {
		t.Logf("Error in Meridian request: %v", bifrostErr)
	}
...
func TestPostLLMHookResponsesStreamDelta(t *testing.T) {
	...
	for i, chunk := range chunks {
		...
		if bifrostErr != nil {
			t.Fatalf("chunk %d: unexpected meridian error: %v", i, bifrostErr)
		}
...
func TestPostLLMHookResponsesStreamNonDeltaPassthrough(t *testing.T) {
	...
	for _, eventType := range nonDeltaTypes {
		...
		if bifrostErr != nil {
			t.Fatalf("event %q: unexpected meridian error: %v", eventType, bifrostErr)
		}
		if result.ResponsesStreamResponse.Delta != nil {
...
func TestPostLLMHookResponsesStreamDoesNotMutateOriginal(t *testing.T) {
	...
	if bifrostErr != nil {
		t.Fatalf("unexpected meridian error: %v", bifrostErr)
	}
...
func TestPostLLMHookResponsesStreamPerRequest(t *testing.T) {
	...
	if bifrostErr != nil {
		t.Fatalf("unexpected meridian error (no-op path): %v", bifrostErr)
	}
	...
	if bifrostErr != nil {
		t.Fatalf("unexpected meridian error (enabled path): %v", bifrostErr)
	}
	if !plugin.isValidJSON(*result.ResponsesStreamResponse.Delta) {
...
func TestJsonParserPluginResponsesStreamEndToEnd(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to init meridian: %v", err)
	}
	defer client.Shutdown()
