...
func buildContextAttrs(ctx context.Context, resp *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) []attribute.KeyValue {
	requestType, provider, originalModel, resolvedModel := meridian.GetResponseFields(resp, bifrostErr)
	model := originalModel
	if resolvedModel != "" {
		model = resolvedModel
	}
	...
	return BuildBifrostAttributes(
		...
		string(requestType),
		meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyID),
		meridian.GetStringFromContext(ctx, schemas.BifrostContextKeyGovernanceVirtualKeyName),
		meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedKeyID),
		meridian.GetStringFromContext(ctx, schemas.BifrostContextKeySelectedKeyName),
		meridian.GetIntFromContext(ctx, schemas.BifrostContextKeyFallbackIndex),
		teamIDs,
		teamNames,
		customerIDs,
		customerNames,
