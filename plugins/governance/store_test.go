...
func TestGovernanceStore_RoutingRules_CreateAndRetrieve(t *testing.T) {
	...
	rule1 := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: meridian.Ptr("openai"), Model: meridian.Ptr("gpt-4"), Weight: 1.0},
		},
		Fallbacks:       nil,
	...
	rule2 := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: meridian.Ptr("azure"), Weight: 1.0},
		},
		Fallbacks:       nil,
...
func TestGovernanceStore_RoutingRules_MultipleScopes(t *testing.T) {
	...
	globalRule := &configstoreTables.TableRoutingRule{
		ID: "1", Name: "Global", Scope: "global", ScopeID: nil, Priority: 10, Enabled: meridian.Ptr(true),
	}
	customerRule := &configstoreTables.TableRoutingRule{
		ID: "2", Name: "Customer", Scope: "customer", ScopeID: &customerID, Priority: 20, Enabled: meridian.Ptr(true),
	}
	teamRule := &configstoreTables.TableRoutingRule{
		ID: "3", Name: "Team", Scope: "team", ScopeID: &teamID, Priority: 30, Enabled: meridian.Ptr(true),
	}
