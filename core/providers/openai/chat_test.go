...
func TestOpenAIChatRequest_StripsCitationTextFromAnnotations(t *testing.T) {
	...
	require.NotContains(t, s, `"text"`, "Meridian-extension citation text must not reach the OpenAI wire")
...
func TestOpenAIChatRequest_StripsWebSearchOptionsFilters(t *testing.T) {
	...
	require.NotContains(t, s, `"filters"`, "Meridian-extension filters must not reach the OpenAI wire")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, converted[0].ChatAssistantMessage.Reasoning,
				"replayed reasoning must reach the Meridian schema regardless of spelling")
