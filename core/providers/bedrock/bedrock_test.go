...
import (
	...
	"github.com/neria-cloud/meridian-base/core/internal/llmtests"
...
func TestGuardrailConfigRequestRoundTrip(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result.GuardrailConfig, "GuardrailConfig must survive the round-trip through Meridian")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedBedrock, actual.StopReason,
				"Meridian stop reason %v should reverse-map to Bedrock %q", tt.stopReason, tt.expectedBedrock)
		})
	}
}
...
func TestDocumentFormatResponsesPathRoundTrip(t *testing.T) {
	...
	for _, format := range formats {
		t.Run(format, func(t *testing.T) {
			...
			require.NotEmpty(t, bifrostMessages, "expected at least one Meridian message")
...
func TestBedrockSearchResultToolResultRoundTrip(t *testing.T) {
							...
							Content: []bedrock.BedrockContentBlock{
								{
									SearchResult: &bedrock.BedrockSearchResultBlock{
										...
										Content: []bedrock.BedrockSearchResultContent{
											{Text: "Apptio is a company that makes calls to Bedrock using passthrough APIs via Meridian"},
										},
	...
	assert.Equal(t, "Apptio is a company that makes calls to Bedrock using passthrough APIs via Meridian", got.Content[0].Text)
