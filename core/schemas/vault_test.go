...
func TestStoreVaultSecretVar_StoresPlaintext(t *testing.T) {
	...
	if err := StoreVaultSecretVar(context.Background(), "meridian/tbl/id/value", e); err != nil {
		t.Fatalf("StoreVaultSecretVar: %v", err)
	}
	if got := stored["meridian/tbl/id/value"]; got != "secret-key" {
...
func TestStoreVaultSecretVar_NoOps(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stored := withStubVaultStore(t)
			if err := StoreVaultSecretVar(context.Background(), "meridian/tbl/id/f", tc.e); err != nil {
				t.Fatalf("StoreVaultSecretVar: %v", err)
			}
			if len(stored) != 0 {
...
func TestRemoveOwnedVaultSecretVars_SkipsFragmentRefs(t *testing.T) {
	...
	RemoveOwnedVaultSecretVars(context.Background(), "meridian/m/1", m)

	if len(removed) != 1 || removed[0] != "meridian/m/1/normal" {
		t.Errorf("removed = %v, want only [meridian/m/1/normal]", removed)
	}
}
...
func TestStoreOwnedVaultSecretVars_WalksFields(t *testing.T) {
	...
	if err := StoreOwnedVaultSecretVars(context.Background(), "meridian/m/1", m); err != nil {
		t.Fatalf("StoreOwnedVaultSecretVars: %v", err)
	}

	want := map[string]string{
		"meridian/m/1/plain_col": "p1",
		"meridian/m/1/ptr_col":   "p2",
		"meridian/m/1/snake":     "p3",
	}
...
func TestStoreOwnedVaultSecretVars_WalksMap(t *testing.T) {
	...
	if got := stored["meridian/m/1/headers/Authorization"]; got != "secret-token" {
	...
	if env := m.Headers["X-Env"]; env.IsFromSecret() && env.GetRawRef() == "env.X" {
		if stored["meridian/m/1/headers/X-Env"] != "" {
...
func TestRemoveOwnedVaultSecretVars_WalksMap(t *testing.T) {
	...
	if len(removed) != 1 || removed[0] != "meridian/m/1/headers/Owned" {
		t.Errorf("removed = %v, want only [meridian/m/1/headers/Owned]", removed)
	}
}
