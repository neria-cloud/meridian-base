...
func InitMCPServerPaths(t *testing.T) {
	...
	t.Logf("  - Meridian Root: %s", mcpServerPaths.BifrostRoot)
...
func SetInternalClientAutoExecute(manager *mcp.MCPManager, toolNames []string) error {
	...
	if internalClient == nil {
		return fmt.Errorf("internal meridian client not found")
	}
...
func GetBifrostRoot(t *testing.T) string {
	...
	for {
		...
		if parent == dir {
			...
			t.Fatal("could not find meridian root (go.mod not found)")
		}
		dir = parent
	}
}
...
func setupBifrost(t *testing.T) *meridian.Bifrost {
	t.Helper()

	account := &testAccount{}

	// Create bifrost instance
	bifrostInstance, err := meridian.Init(context.Background(), schemas.BifrostConfig{
		Account: account,
		Logger:  meridian.NewDefaultLogger(schemas.LogLevelError),
	})
	require.NoError(t, err, "failed to create meridian instance")
...
func AssertBifrostErrorContains(t *testing.T, bifrostErr *schemas.BifrostError, expectedMessage string) {
	t.Helper()
	require.NotNil(t, bifrostErr, "meridian error should not be nil")
	require.NotNil(t, bifrostErr.Error, "meridian error.Error should not be nil")
...
func CreateTempTestFile(t *testing.T, content string) string {
	t.Helper()
	tmpFile, err := os.CreateTemp("", "meridian-test-*")
	require.NoError(t, err)
