...
func RunWebSocketResponsesTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("WebSocketResponses", func(t *testing.T) {
		...
		if provider == nil {
			t.Fatalf("provider %s not found in meridian client", testConfig.Provider)
		}
