...
func RunOpus45ReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, config OpusReasoningTestConfig) {
		...
		t.Run("ResponsesAPI", func(t *testing.T) {
			...
			responsesReq := &schemas.BifrostResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: meridian.Ptr(4000),
					Reasoning: &schemas.ResponsesParametersReasoning{
						Effort: meridian.Ptr("high"),
					},
		...
		t.Run("ChatCompletionsAPI", func(t *testing.T) {
			...
			chatReq := &schemas.BifrostChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: meridian.Ptr(4000),
					Reasoning: &schemas.ChatReasoning{
						Effort:    meridian.Ptr("high"),
						MaxTokens: meridian.Ptr(2000), // Budget tokens for Opus 4.5
					},
				},
				Fallbacks: config.Fallbacks,
			}
...
func RunOpus46ReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, config OpusReasoningTestConfig) {
			...
			t.Run("ResponsesAPI_Effort_"+effort, func(t *testing.T) {
				...
				responsesReq := &schemas.BifrostResponsesRequest{
					...
					Params: &schemas.ResponsesParameters{
						MaxOutputTokens: meridian.Ptr(4000),
						Reasoning: &schemas.ResponsesParametersReasoning{
							Effort: meridian.Ptr(effort), // Adaptive thinking uses effort parameter
						},
						Include: []string{"reasoning.encrypted_content"},
					},
					Fallbacks: config.Fallbacks,
		...
		t.Run("ChatCompletionsAPI", func(t *testing.T) {
			...
			chatReq := &schemas.BifrostChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: meridian.Ptr(4000),
					Reasoning: &schemas.ChatReasoning{
						Effort: meridian.Ptr("high"), // Opus 4.6 uses adaptive thinking with effort
...
func RunOpus46MultiTurnReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, config OpusReasoningTestConfig) {
	if config.SkipOpus46 {
...
func RunAllOpusReasoningTests(t *testing.T, client *meridian.Bifrost, ctx context.Context, config OpusReasoningTestConfig) {
