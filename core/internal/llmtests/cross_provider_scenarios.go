...
func NewOpenAIJudge(client *meridian.Bifrost, judgeModel string, t *testing.T) *OpenAIJudge {
	return &OpenAIJudge{
		client:     client,
  ...
  "fatal_issues": ["serious problems"]
	...
	request := &schemas.BifrostChatRequest{
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: meridian.Ptr(600),
			Temperature:         meridian.Ptr(0.1),
		},
	}
...
func NewOpenAIConversationDriver(client *meridian.Bifrost, driverModel string, t *testing.T) *OpenAIConversationDriver {
...
JSON response:
{
  ...
  "test_focus": "what capability this tests"
	...
	llmRequest := &schemas.BifrostChatRequest{
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: meridian.Ptr(300),
...
func RunCrossProviderScenarioTest(t *testing.T, client *meridian.Bifrost, ctx *schemas.BifrostContext, config CrossProviderTestConfig, scenario CrossProviderScenario, useResponsesAPI bool) {
...
func RunCrossProviderConsistencyTest(t *testing.T, client *meridian.Bifrost, ctx *schemas.BifrostContext, config CrossProviderTestConfig, useResponsesAPI bool) {
	apiType := "Chat Completions"
	if useResponsesAPI {
		apiType = "Responses API"
	}
...
func executeStepWithProvider(t *testing.T, client *meridian.Bifrost, ctx *schemas.BifrostContext,
	...
	if useResponsesAPI {
		...
		request := &schemas.BifrostResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: meridian.Ptr(300),
				Temperature:     meridian.Ptr(0.7),
			},
		}
