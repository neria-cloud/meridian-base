...
func getBifrost(ctx context.Context) (*meridian.Bifrost, error) {
	account := ComprehensiveTestAccount{}

	// Initialize Bifrost
	b, err := meridian.Init(ctx, schemas.BifrostConfig{
		Account: &account,
...
func SetupTest() (*meridian.Bifrost, context.Context, context.CancelFunc, error) {
