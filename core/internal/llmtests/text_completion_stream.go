...
func RunTextCompletionStreamTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("TextCompletionStreamParameters", func(t *testing.T) {
		...
		parameterTests := []struct {
			...
			{
				name:        "HighCreativity",
				temperature: meridian.Ptr(0.9),
				maxTokens:   meridian.Ptr(100),
				topP:        meridian.Ptr(0.9),
			},
			{
				name:        "LowCreativity",
				temperature: meridian.Ptr(0.1),
				maxTokens:   meridian.Ptr(50),
				topP:        meridian.Ptr(0.5),
			},
			{
				name:        "Balanced",
				temperature: meridian.Ptr(0.5),
				maxTokens:   meridian.Ptr(75),
				topP:        meridian.Ptr(0.8),
			},
		}
