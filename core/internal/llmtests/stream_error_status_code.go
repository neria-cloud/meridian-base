...
func RunStreamErrorStatusCodeTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("StreamErrorStatusCode", func(t *testing.T) {
		...
		invalidModel := "meridian-nonexistent-model-for-testing-12345"
		...
		t.Run("ChatCompletionStream", func(t *testing.T) {
			...
			request := &schemas.BifrostChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: meridian.Ptr(10),
				},
			}
			...
			if bifrostErr.IsBifrostError {
				...
				t.Logf("⚠️  Error is a Meridian error (not provider error) with status %d — this may indicate model validation happened before the provider call", statusCode)
			}
		...
		t.Run("ResponsesStream", func(t *testing.T) {
			...
			request := &schemas.BifrostResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: meridian.Ptr(10),
				},
			}
