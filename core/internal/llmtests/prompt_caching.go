	...
	return []schemas.ChatTool{
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Retrieve account information for a given account ID"),
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Calculate the cost for cloud resource usage based on service type and quantity"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Check the current status and health of the API service"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Create a new support ticket for a customer issue"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Retrieve documentation for a specific service or feature"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Get a list of available cloud regions for deployment"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("service_type", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Estimate the monthly cost for a cloud deployment configuration"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Validate an API key and return its permissions and status"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Retrieve usage analytics and metrics for an account"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("account_id", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: meridian.Ptr("Check compliance status and certifications for security and regulatory requirements"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
...
func RunPromptCachingToolBlocksTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PromptCachingToolBlocks", func(t *testing.T) {
		...
		systemMsg := schemas.ResponsesMessage{
			Type: meridian.Ptr(schemas.ResponsesMessageTypeMessage),
			Role: meridian.Ptr(schemas.ResponsesInputMessageRoleSystem),
			Content: &schemas.ResponsesMessageContent{
				ContentBlocks: []schemas.ResponsesMessageContentBlock{
					{
						Type:         schemas.ResponsesInputMessageContentBlockTypeText,
						Text:         meridian.Ptr(longSharedPrefix),
		...
		userMsg1 := schemas.ResponsesMessage{
			Type: meridian.Ptr(schemas.ResponsesMessageTypeMessage),
			Role: meridian.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: meridian.Ptr("What's the weather in San Francisco?"),
			},
		}

		// Assistant responds with a tool call (function_call with cache_control)
		toolCallMsg := schemas.ResponsesMessage{
			Type:         meridian.Ptr(schemas.ResponsesMessageTypeFunctionCall),
			Status:       meridian.Ptr("completed"),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID:    meridian.Ptr("call_weather_001"),
				Name:      meridian.Ptr("weather"),
				Arguments: meridian.Ptr(`{"location":"San Francisco"}`),
			},
		}

		// Tool result (function_call_output with cache_control)
		toolResultMsg := schemas.ResponsesMessage{
			Type:         meridian.Ptr(schemas.ResponsesMessageTypeFunctionCallOutput),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID: meridian.Ptr("call_weather_001"),
				Output: &schemas.ResponsesToolMessageOutputStruct{
					ResponsesToolCallOutputStr: meridian.Ptr(`{"temperature": 18, "unit": "celsius", "condition": "partly cloudy", "humidity": 72}`),
				},
			},
		}
		...
		userMsg2 := schemas.ResponsesMessage{
			...
			Role: meridian.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: meridian.Ptr("Summarize the weather information you received."),
			},
		}
		...
		responsesReq := &schemas.BifrostResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: meridian.Ptr(200),
			},
		}
...
func RunPromptCachingMultipleToolCallsTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.PromptCaching {
	...
	t.Run("PromptCachingMultipleToolCalls", func(t *testing.T) {
		...
		makeAssistantMsg := func(text string) schemas.ResponsesMessage {
			return schemas.ResponsesMessage{
				Type: meridian.Ptr(schemas.ResponsesMessageTypeMessage),
				Role: meridian.Ptr(schemas.ResponsesInputMessageRoleAssistant),
				Content: &schemas.ResponsesMessageContent{
					ContentStr: meridian.Ptr(text),
				},
			}
		...
		makeToolCall := func(callID, name, args string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				Status: meridian.Ptr("completed"),
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID:    meridian.Ptr(callID),
					Name:      meridian.Ptr(name),
					Arguments: meridian.Ptr(args),
				},
			}
			if withCacheControl {
		...
		makeToolResult := func(callID, output string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID: meridian.Ptr(callID),
					Output: &schemas.ResponsesToolMessageOutputStruct{
						ResponsesToolCallOutputStr: meridian.Ptr(output),
					},
				},
			}
			if withCacheControl {
...
func RunPromptCachingTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for i, query := range testQueries {
			t.Run(query.name, func(t *testing.T) {
				userMessage := schemas.ChatMessage{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: meridian.Ptr(query.message),
					},
				}
				...
				chatReq := &schemas.BifrostChatRequest{
					...
					Params: &schemas.ChatParameters{
						...
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: meridian.Ptr("auto"),
						},
					},
...
func RunPromptCachingMultiTurnTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SimpleChat {
			...
			t.Run(turnName, func(t *testing.T) {
				...
				chatReq := &schemas.BifrostChatRequest{
					...
					Params: &schemas.ChatParameters{
						Tools: tools,
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: meridian.Ptr("none"),
						},
					},
					Fallbacks: testConfig.Fallbacks,
				}
