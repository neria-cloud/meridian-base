...
func RunResponsesReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesReasoning", func(t *testing.T) {
		...
		responsesReq := &schemas.BifrostResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: meridian.Ptr(25000),
				// Configure reasoning-specific parameters
				Reasoning: &schemas.ResponsesParametersReasoning{
					Effort: meridian.Ptr("high"), // High effort for complex reasoning
...
func RunChatCompletionReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ChatCompletionReasoning", func(t *testing.T) {
		...
		chatReq := &schemas.BifrostChatRequest{
			...
			Params: &schemas.ChatParameters{
				MaxCompletionTokens: meridian.Ptr(1800),
				// Configure reasoning-specific parameters
				Reasoning: &schemas.ChatReasoning{
					Effort:    meridian.Ptr("high"), // High effort for complex reasoning
					MaxTokens: meridian.Ptr(1500),   // Maximum tokens for reasoning output
				},
			},
			Fallbacks: testConfig.Fallbacks,
		}
...
func RunMultiTurnReasoningTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
