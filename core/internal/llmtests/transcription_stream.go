...
func RunTranscriptionStreamTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunTranscriptionStreamAdvancedTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.TranscriptionStream {
		t.Logf("Transcription streaming not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("TranscriptionStreamAdvanced", func(t *testing.T) {
		...
		t.Run("WithCustomPrompt_Streaming", func(t *testing.T) {
			...
			request := &schemas.BifrostTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language: meridian.Ptr("en"),
					Prompt:   meridian.Ptr("This audio contains technical terms, proper nouns, and streaming-related vocabulary."),
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
