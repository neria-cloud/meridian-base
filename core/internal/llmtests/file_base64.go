			...
			ContentBlocks: []schemas.ChatContentBlock{
				{Type: schemas.ChatContentBlockTypeText, Text: meridian.Ptr(text)},
				{
					...
					File: &schemas.ChatInputFile{
						FileData: meridian.Ptr(documentBase64),
						Filename: meridian.Ptr("test_document.pdf"),
					},
				},
			},
		},
...
func RunFileBase64Test(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunFileBase64ChatCompletionsTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.BifrostChatResponse, *schemas.BifrostError) {
			...
			chatReq := &schemas.BifrostChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: meridian.Ptr(500),
				},
...
func RunFileBase64ResponsesTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.BifrostResponsesResponse, *schemas.BifrostError) {
			...
			responsesReq := &schemas.BifrostResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: meridian.Ptr(500),
				},
				Fallbacks: testConfig.Fallbacks,
			}
