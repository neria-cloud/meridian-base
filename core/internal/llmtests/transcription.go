...
func RunTranscriptionTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Transcription", func(t *testing.T) {
		...
		roundTripCases := []struct {
		...
		}{
			{
				...
				responseFormat: meridian.Ptr("json"),
			},
			{
				...
				if testConfig.Provider == schemas.HuggingFace && strings.HasPrefix(testConfig.TranscriptionModel, "fal-ai/") {
					...
					transcriptionRequest = &schemas.BifrostTranscriptionRequest{
						...
						Params: &schemas.TranscriptionParameters{
							Language:       meridian.Ptr("en"),
							Format:         meridian.Ptr("mp3"),
...
func RunTranscriptionAdvancedTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Transcription {
		t.Logf("Transcription not supported for provider %s", testConfig.Provider)
		return
	}
		...
		t.Run("WithCustomParameters", func(t *testing.T) {
			...
			request := &schemas.BifrostTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language:       meridian.Ptr("en"),
					Format:         &audioFormat,
					Prompt:         meridian.Ptr("This audio contains technical terminology and proper nouns."),
					ResponseFormat: meridian.Ptr("json"), // Use json instead of verbose_json for whisper-1
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
