...
func RunPassthroughExtraParamsTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughExtraParams", func(t *testing.T) {
		...
		if response.ExtraFields.RawRequest == nil {
			...
			t.Logf("   Check Meridian logs for the raw request body sent to provider")
