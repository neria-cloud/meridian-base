...
func GetSampleChatTool(toolName SampleToolType) *schemas.ChatTool {
	...
	return &schemas.ChatTool{
		...
		Function: &schemas.ChatToolFunction{
			...
			Description: meridian.Ptr(description),
...
func GetSampleResponsesTool(toolName SampleToolType) *schemas.ResponsesTool {
	...
	return &schemas.ResponsesTool{
		Type:        "function",
		Name:        meridian.Ptr(toolDisplayName),
		Description: meridian.Ptr(description),
...
func CreateBasicChatMessage(content string) schemas.ChatMessage {
	return schemas.ChatMessage{
		...
		Content: &schemas.ChatMessageContent{
			ContentStr: meridian.Ptr(content),
		},
	}
}
	...
	return schemas.ChatMessage{
		...
		Content: &schemas.ChatMessageContent{
			ContentBlocks: []schemas.ChatContentBlock{
				{Type: schemas.ChatContentBlockTypeText, Text: meridian.Ptr(text)},
	...
	return schemas.ResponsesMessage{
		...
		Content: &schemas.ResponsesMessageContent{
			ContentBlocks: []schemas.ResponsesMessageContentBlock{
				{Type: schemas.ResponsesInputMessageContentBlockTypeText, Text: meridian.Ptr(text)},
				{
					Type: schemas.ResponsesInputMessageContentBlockTypeImage,
					ResponsesInputMessageContentBlockImage: &schemas.ResponsesInputMessageContentBlockImage{
						ImageURL: meridian.Ptr(imageURL),
					},
				},
			},
		},
...
func CreateAudioChatMessage(text, audioData string, audioFormat string) schemas.ChatMessage {
	format := meridian.Ptr(audioFormat)
	return schemas.ChatMessage{
		Role: schemas.ChatMessageRoleUser,
		Content: &schemas.ChatMessageContent{
			ContentBlocks: []schemas.ChatContentBlock{
...
func CreateToolChatMessage(content string, toolCallID string) schemas.ChatMessage {
	return schemas.ChatMessage{
		...
		ChatToolMessage: &schemas.ChatToolMessage{
			ToolCallID: meridian.Ptr(toolCallID),
		},
	}
}
...
func GenerateTTSAudioForTest(ctx context.Context, t *testing.T, client *meridian.Bifrost, provider schemas.ModelProvider, ttsModel string, text string, voiceType string, format string) ([]byte, string) {
	...
	suffix := "." + format
	f, cerr := os.CreateTemp("", "meridian-tts-*"+suffix)
	if cerr != nil {
