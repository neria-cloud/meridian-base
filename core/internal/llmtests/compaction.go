...
func RunCompactionTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunExternalCompactionTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ExternalCompaction", func(t *testing.T) {
		...
		t.Run("CompactedOutputUsableInFollowUp", func(t *testing.T) {
			...
			followUpResp, followUpErr := client.ResponsesRequest(bfCtx2, &schemas.BifrostResponsesRequest{
				...
				Model:    model,
				Input:    followUpInput,
				Params:   &schemas.ResponsesParameters{MaxOutputTokens: meridian.Ptr(100)},
			})
			if followUpErr != nil {
