...
func RunResponsesStreamTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ResponsesStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.BifrostResponsesRequest{
				...
				Input:    messages,
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: meridian.Ptr(1800),
