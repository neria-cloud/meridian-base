...
func RunToolCallsTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunToolCallsWithEmptyPropertiesTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
			...
			chatReq := &schemas.BifrostChatRequest{
				...
				Params: &schemas.ChatParameters{
					...
					ToolChoice: &schemas.ChatToolChoice{
						ChatToolChoiceStr: meridian.Ptr("required"),
					},
				},
				Fallbacks: testConfig.Fallbacks,
			}
			...
			responsesReq := &schemas.BifrostResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					...
					ToolChoice: &schemas.ResponsesToolChoice{
						ResponsesToolChoiceStr: meridian.Ptr("required"),
					},
				},
			}
			return client.ResponsesRequest(bfCtx, responsesReq)
...
func RunToolCallsWithNilPropertiesTest(t *testing.T, client *meridian.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
