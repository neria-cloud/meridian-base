...
datasheet from local files instead of fetching them from `meridian.neria.cloud`. Use
it when the host has no outbound internet access, or when an HTTP proxy blocks
DNS resolution of `meridian.neria.cloud` (see issue #4305).
...
```bash
curl -fsSL https://meridian.neria.cloud/datasheet -o pricing.json
curl -fsSL https://meridian.neria.cloud/datasheet/model-parameters -o model-parameters.json
```
