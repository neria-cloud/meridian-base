...
func main() {
	extra := headerList{}
	url := flag.String("url", "http://localhost:8080/mcp", "Meridian /mcp endpoint")
...
func initRequest() mcp.InitializeRequest {
	...
	req.Params.ClientInfo = mcp.Implementation{Name: "meridian-mcp-test-client", Version: "0.1.0"}
...
func authorize(cfg *config, h *transport.OAuthHandler) error {
	...
	if h.GetClientID() == "" {
		...
		err := h.RegisterClient(regCtx, "meridian-mcp-test-client")
		cancel()
		if err != nil {
