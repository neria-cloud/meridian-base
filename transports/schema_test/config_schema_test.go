...
func TestSchemaSCIMConfigValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			config: `{"scim_config":{"enabled":true,"provider":"zitadel","config":{"domain":"acme.zitadel.cloud","clientId":"meridian"}}}`,
		},
		{
		...
		},
		{
			...
			config: `{"scim_config":{"enabled":true,"provider":"google","config":{"domain":"company.com","clientId":"meridian"}}}`,
		},
		{
			name:      "enabled google with credentialMode env but no serviceAccountEnvVar is invalid",
			config:    `{"scim_config":{"enabled":true,"provider":"google","config":{"domain":"company.com","clientId":"meridian","credentialMode":"env"}}}`,
			wantError: true,
		},
		{
					...
					"keys": [
						{
							...
							"bedrock_key_config": {
								...
								"session_name": "meridian-cross-account",
					...
					"keys": [
						{
							...
							"bedrock_key_config": {
								...
								"batch_s3_config": {
									"buckets": [
										{"bucket_name": "my-batch-bucket", "prefix": "meridian/", "is_default": true},
