...
func (h *WebhookHandler) createWebhookEndpoint(ctx *fasthttp.RequestCtx) {
	...
	if err := h.manager.ReloadWebhookEndpoint(ctx, endpoint.ID); err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to reload webhook endpoint in memory: %v, please restart meridian to sync with the database", err))
		return
	}
...
func (h *WebhookHandler) updateWebhookEndpoint(ctx *fasthttp.RequestCtx) {
	...
	if !ok {
		SendError(ctx, fasthttp.StatusInternalServerError, "Webhook endpoint missing from memory after reload, please restart meridian to sync with the database")
		return
	}
