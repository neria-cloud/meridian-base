...
import (
	...
	openaiProvider "github.com/neria-cloud/meridian-base/core/providers/openai"
...
func mapRealtimeWireErrorFields(bifrostErr *schemas.BifrostError) (string, string, string, string) {
	...
	for _, value := range values {
		...
		case meridian.IsRateLimitErrorMessage(lower):
...
func shouldGracefullyDisconnectRealtime(bifrostErr *schemas.BifrostError) bool {
	...
	for _, value := range values {
		...
		if isBudgetOrBillingError(lower) || meridian.IsRateLimitErrorMessage(lower) {
			return true
		}
	}
...
func newBifrostErrorFromRealtimeError(
	...
	for _, value := range values {
		...
		case strings.Contains(lower, "invalid_request_error"):
			statusCode = 400
		case isBudgetOrBillingError(lower), meridian.IsRateLimitErrorMessage(lower):
			statusCode = 429
		}
	}
