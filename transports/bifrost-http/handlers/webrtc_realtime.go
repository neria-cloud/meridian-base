...
import (
	...
	"github.com/neria-cloud/meridian-base/plugins/modelcatalogresolver"
...
func NewWebRTCRealtimeHandler(client *meridian.Bifrost, config *lib.Config) *WebRTCRealtimeHandler {
...
func (h *WebRTCRealtimeHandler) establishRelay(
	...
	providerToBrowserTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "meridian-provider-audio")
	if err != nil {
		relay.close()
	...
	browserToProviderTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "meridian-browser-audio")
	if err != nil {
		relay.close()
