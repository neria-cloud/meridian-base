...
import (
	...
	governanceplugin "github.com/neria-cloud/meridian-base/plugins/governance"
...
func NewProviderHandler(modelsManager ModelsManager, inMemoryStore *lib.Config, client *meridian.Bifrost) *ProviderHandler {
...
func (h *ProviderHandler) addProvider(ctx *fasthttp.RequestCtx) {
	...
	if payload.CustomProviderConfig != nil {
		...
		if meridian.IsStandardProvider(payload.Provider) {
		...
		if !meridian.IsSupportedBaseProvider(payload.CustomProviderConfig.BaseProviderType) {
	...
	if payload.NetworkConfig != nil {
		...
		if payload.NetworkConfig.BaseURL != "" {
			if err := meridian.ValidateExternalURL(payload.NetworkConfig.BaseURL, payload.NetworkConfig.AllowPrivateNetwork); err != nil {
...
func (h *ProviderHandler) updateProvider(ctx *fasthttp.RequestCtx) {
	...
	if nc.BaseURL != "" {
		if err := meridian.ValidateExternalURL(nc.BaseURL, nc.AllowPrivateNetwork); err != nil {
			SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("Invalid base URL: %v", err))
			return
		}
	}
