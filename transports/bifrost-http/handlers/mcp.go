...
func NewMCPHandler(mcpManager MCPManager, governanceManager GovernanceManager, client *meridian.Bifrost, store *lib.Config, oauthHandler *OAuthHandler) *MCPHandler {
	return &MCPHandler{
		client:            client,
		store:             store,
		mcpManager:        mcpManager,
...
func (h *MCPHandler) getMCPClientsPaginated(ctx *fasthttp.RequestCtx, params configstore.MCPClientsQueryParams, states []string) {
	...
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to get MCP clients from Meridian: %v", err))
		return
	}
	...
	if err := h.mcpManager.AddMCPClient(bifrostCtx, schemasConfig); err != nil {
		...
		if h.store.ConfigStore != nil {
			if err := h.store.ConfigStore.DeleteMCPClientConfig(ctx, schemasConfig.ID); err != nil {
				logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart meridian to keep core and database in sync", err))
				SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart meridian to keep core and database in sync", err))
				return
			}
		}
	...
	if err := h.mcpManager.UpdateMCPClient(ctx, id, schemasConfig); err != nil {
		...
		if h.store.ConfigStore != nil && oldDBConfig != nil {
			if rollbackErr := h.store.ConfigStore.UpdateMCPClientConfig(ctx, id, oldDBConfig); rollbackErr != nil {
				logger.Error(fmt.Sprintf("Failed to rollback MCP client DB update: %v. please restart meridian to keep core and database in sync", rollbackErr))
			}
		}
			...
			if err := h.mcpManager.AddMCPClient(bifrostCtx, mcpClientConfig); err != nil {
				...
				if h.store.ConfigStore != nil {
					if delErr := h.store.ConfigStore.DeleteMCPClientConfig(ctx, mcpClientConfig.ID); delErr != nil {
						logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart meridian to keep core and database in sync", delErr))
						SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart meridian to keep core and database in sync", delErr))
						return
					}
				}
				SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to register MCP client: %v", err))
