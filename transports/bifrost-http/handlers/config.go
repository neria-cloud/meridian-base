...
import (
	...
	"github.com/neria-cloud/meridian-base/core/network"
...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	if frameworkConfig == nil {
		frameworkConfig = &configstoreTables.TableFrameworkConfig{
			ID:                     0,
			PricingURL:             meridian.Ptr(modelcatalog.DefaultPricingURL),
			PricingSyncInterval:    meridian.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds())),
			ModelParametersURL:     meridian.Ptr(modelcatalog.DefaultModelParametersURL),
			MCPLibraryURL:          meridian.Ptr(modelcatalog.DefaultMCPLibraryURL),
			MCPLibrarySyncInterval: meridian.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds())),
			LiveModelsSyncInterval: meridian.Ptr(int64(modelcatalog.DefaultLiveModelsSyncInterval.Seconds())),
		}
	}
	// Handling individual nil cases
	if frameworkConfig.PricingURL == nil {
		frameworkConfig.PricingURL = meridian.Ptr(modelcatalog.DefaultPricingURL)
	}
	if frameworkConfig.PricingSyncInterval == nil {
		frameworkConfig.PricingSyncInterval = meridian.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds()))
	}
	if frameworkConfig.ModelParametersURL == nil {
		frameworkConfig.ModelParametersURL = meridian.Ptr(modelcatalog.DefaultModelParametersURL)
	}
	if frameworkConfig.MCPLibraryURL == nil {
		frameworkConfig.MCPLibraryURL = meridian.Ptr(modelcatalog.DefaultMCPLibraryURL)
	}
	if frameworkConfig.MCPLibrarySyncInterval == nil {
		frameworkConfig.MCPLibrarySyncInterval = meridian.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds()))
	}
	if frameworkConfig.LiveModelsSyncInterval == nil {
		frameworkConfig.LiveModelsSyncInterval = meridian.Ptr(int64(modelcatalog.DefaultLiveModelsSyncInterval.Seconds()))
	}
...
func checkURLAccessibility(rawURL string) error {
	...
	if err := meridian.ValidateExternalURL(rawURL, true); err != nil {
