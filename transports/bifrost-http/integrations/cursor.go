...
func cursorChunkID(extras *schemas.BifrostResponseExtraFields) string {
	return "chatcmpl-meridian-" + strconv.Itoa(extras.ChunkIndex)
}
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.BifrostContext, req interface{}) (*schemas.BifrostRequest, error) {
				if openaiReq, ok := req.(*openai.OpenAIResponsesRequest); ok {
					...
					if bifrostReq == nil {
						return nil, errors.New("meridian responses request conversion returned nil")
					}
...
func NewCursorRouter(client *meridian.Bifrost, handlerStore lib.HandlerStore, logger schemas.Logger) *CursorRouter {
	routes := []RouteConfig{}
