...
type GenericRouter struct {
	client            *meridian.Bifrost // Bifrost client for executing requests
...
func NewGenericRouter(client *meridian.Bifrost, handlerStore lib.HandlerStore, routes []RouteConfig, passthroughCfg *PassthroughConfig, logger schemas.Logger) *GenericRouter {
...
func (g *GenericRouter) createHandler(config RouteConfig) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		...
		if err != nil {
			g.sendError(ctx, bifrostCtx, config.ErrorConverter, newBifrostError(err, "failed to convert request to Meridian format"))
			return
		}
		if bifrostReq == nil {
...
func (g *GenericRouter) handleNonStreamingRequest(ctx *fasthttp.RequestCtx, config RouteConfig, req interface{}, bifrostReq *schemas.BifrostRequest, bifrostCtx *schemas.BifrostContext) {
	...
	case bifrostReq.ListModelsRequest != nil:
		...
		if listModelsResponse == nil {
			g.sendError(ctx, bifrostCtx, config.ErrorConverter, newBifrostError(nil, "Meridian response is nil after post-request callback"))
			return
		}
