...
func Test_bedrockStreamErrorConverterEncodesEventStreamException(t *testing.T) {
	...
	require.True(t, sendBedrockEventStreamException(reader, eventstream.NewEncoder(), exception, meridian.NewNoOpLogger()))
	reader.Done()
...
func Test_handleStreamingBedrockUnknownErrorResponseFallsBackToEventStreamException(t *testing.T) {
	...
	router := NewGenericRouter(nil, &mockHandlerStore{}, nil, nil, meridian.NewNoOpLogger())
...
func Test_s3PutObjectPostCallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "meridian", string(ctx.Response.Header.Peek("x-amz-request-id")))
