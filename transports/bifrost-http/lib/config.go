...
import (
	...
	mcputils "github.com/neria-cloud/meridian-base/core/mcp/utils"
	"github.com/neria-cloud/meridian-base/core/schemas"
	"github.com/neria-cloud/meridian-base/framework"
	...
	"github.com/neria-cloud/meridian-base/framework/envutils"
	"github.com/neria-cloud/meridian-base/framework/featureflags"
	"github.com/neria-cloud/meridian-base/framework/kvstore"
	"github.com/neria-cloud/meridian-base/framework/logstore"
	"github.com/neria-cloud/meridian-base/framework/mcp_headers"
	"github.com/neria-cloud/meridian-base/framework/mcpcatalog"
	"github.com/neria-cloud/meridian-base/framework/modelcatalog"
	"github.com/neria-cloud/meridian-base/framework/oauth2"
	"github.com/neria-cloud/meridian-base/framework/objectstore"
	plugins "github.com/neria-cloud/meridian-base/framework/plugins"
	"github.com/neria-cloud/meridian-base/framework/vectorstore"
	"github.com/neria-cloud/meridian-base/plugins/compat"
	...
	"github.com/neria-cloud/meridian-base/plugins/maxim"
	...
	return config.ConfigStore.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i := range governanceConfig.RoutingRules {
			for j := range governanceConfig.RoutingRules[i].Targets {
				...
				target.KeyID = meridian.Ptr(keyID)
				target.ProviderKeyName = nil
			}
		}
		...
		for i := range governanceConfig.PricingOverrides {
			...
			override.ProviderKeyID = meridian.Ptr(keyID)
			override.ProviderKeyName = nil
		}
	...
	if len(config.PluginConfigs) == 0 {
		...
		for _, plugin := range configData.Plugins {
			if plugin.Version == nil {
				plugin.Version = meridian.Ptr(int16(1))
			}
	...
	for providerKey, provider := range c.Providers {
		for _, key := range provider.Keys {
			...
			configStoreKey := configstoreTables.TableKey{
				...
				Weight:            meridian.Ptr(key.Weight),
...
func (c *Config) SetBifrostClient(client *meridian.Bifrost) {
	c.muMCP.Lock()
...
func (c *Config) GetMCPClient(id string) (*schemas.MCPClientConfig, error) {
	...
	if c.client == nil {
		return nil, fmt.Errorf("meridian client not set")
	}
...
func (c *Config) AddMCPClient(ctx context.Context, clientConfig *schemas.MCPClientConfig) error {
	if c.client == nil {
		return fmt.Errorf("meridian client not set")
	}
	c.muMCP.Lock()
	defer c.muMCP.Unlock()
	if c.MCPConfig == nil {
...
func ValidateCustomProvider(config configstore.ProviderConfig, provider schemas.ModelProvider) error {
	...
	if meridian.IsStandardProvider(provider) {
	...
	if !meridian.IsSupportedBaseProvider(cpc.BaseProviderType) {
