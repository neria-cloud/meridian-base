...
func TestValidateConfigSchema_CustomSchemaFilePath(t *testing.T) {
	...
	err := ValidateConfigSchema([]byte(`{"$schema":"/opt/meridian/config.schema.json"}`))
	if err != nil {
...
func TestFilePathFromSchemaLocation(t *testing.T) {
	cases := map[string]string{
		"/opt/meridian/config.schema.json":               "/opt/meridian/config.schema.json",
		"file:///opt/meridian/config.schema.json":        "/opt/meridian/config.schema.json",
		...
		"https://meridian.neria.cloud/schema-as-path-only": "https://meridian.neria.cloud/schema-as-path-only",
	}
