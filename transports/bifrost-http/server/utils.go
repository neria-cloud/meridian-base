...
func GetDefaultConfigDir(appDir string) string {
	...
	case "windows":
		...
		if appData := os.Getenv("APPDATA"); appData != "" {
			configDir = filepath.Join(appData, "meridian")
		} else {
			// Fallback to user home directory
			if homeDir, err := os.UserHomeDir(); err == nil {
				configDir = filepath.Join(homeDir, "AppData", "Roaming", "meridian")
			}
		}
	default:
		// Linux, macOS and other Unix-like systems: ~/.config/bifrost
		if homeDir, err := os.UserHomeDir(); err == nil {
			configDir = filepath.Join(homeDir, ".config", "meridian")
		}
	}

	// If we couldn't determine the config directory, fall back to current directory
	if configDir == "" {
		configDir = "./meridian-data"
	}
