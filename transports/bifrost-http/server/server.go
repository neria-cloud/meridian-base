...
import (
	...
	"github.com/neria-cloud/meridian-base/framework/encrypt"
	"github.com/neria-cloud/meridian-base/framework/logstore"
	"github.com/neria-cloud/meridian-base/framework/modelcatalog"
	dynamicPlugins "github.com/neria-cloud/meridian-base/framework/plugins"
	"github.com/neria-cloud/meridian-base/framework/sidekiq"
	"github.com/neria-cloud/meridian-base/framework/temptoken"
	"github.com/neria-cloud/meridian-base/framework/tracing"
	"github.com/neria-cloud/meridian-base/framework/webhooks"
	"github.com/neria-cloud/meridian-base/plugins/governance"
	"github.com/neria-cloud/meridian-base/plugins/governance/complexity"
	"github.com/neria-cloud/meridian-base/plugins/logging"
	"github.com/neria-cloud/meridian-base/plugins/otel"
	"github.com/neria-cloud/meridian-base/plugins/prompts"
	"github.com/neria-cloud/meridian-base/plugins/semanticcache"
	"github.com/neria-cloud/meridian-base/plugins/telemetry"
	"github.com/neria-cloud/meridian-base/transports/bifrost-http/handlers"
	"github.com/neria-cloud/meridian-base/transports/bifrost-http/integrations"
	"github.com/neria-cloud/meridian-base/transports/bifrost-http/lib"
	bfws "github.com/neria-cloud/meridian-base/transports/bifrost-http/websocket"
...
func (s *BifrostHTTPServer) ReloadProvider(ctx context.Context, provider schemas.ModelProvider) (*tables.TableProvider, error) {
	...
	if s.Client == nil {
		return nil, fmt.Errorf("meridian client not found")
	}
...
func (s *BifrostHTTPServer) RemoveProvider(ctx context.Context, provider schemas.ModelProvider) error {
	...
	if err != nil && !errors.Is(err, lib.ErrNotFound) {
		logger.Error("failed to remove provider from config: %v. Client and config may be out of sync, please restart meridian", err)
		return fmt.Errorf("failed to remove provider from config: %w. Client and config may be out of sync, please restart meridian", err)
	}
	governancePlugin, err := s.getGovernancePlugin()
	if err != nil {
		return err
...
func (s *BifrostHTTPServer) FetchAndStoreLiveForKey(ctx context.Context, provider schemas.ModelProvider, keyID string) {
	...
	go func() {
		...
		if bfErr != nil {
			logger.Warn("filtered list-models failed for provider %s key %s: %v: falling back onto the static datasheet", provider, keyID, meridian.GetErrorMessage(bfErr))
	...
	go func() {
		...
		if bfErr != nil {
			logger.Warn("unfiltered list-models failed for provider %s key %s: %v: falling back onto the static datasheet", provider, keyID, meridian.GetErrorMessage(bfErr))
			return
		}
		if resp == nil {
			return
...
func (s *BifrostHTTPServer) SyncLoadedPlugin(ctx context.Context, name string, plugin schemas.BasePlugin, placement *schemas.PluginPlacement, order *int) error {
	...
	if err := s.Client.ReloadPlugin(plugin, InferPluginTypes(plugin)); err != nil {
		return s.updatePluginErrorStatus(plugin.GetName(), "reloading meridian config for", err)
	}
...
func (s *BifrostHTTPServer) RemovePlugin(ctx context.Context, displayName string) error {
	...
	if err := s.Client.RemovePlugin(name, InferPluginTypes(plugin)); err != nil {
		logger.Warn("failed to reload meridian config after plugin removal: %v", err)
	}
...
func (s *BifrostHTTPServer) Bootstrap(ctx context.Context) error {
	...
	account := lib.NewBaseAccount(s.Config)
	s.Client, err = meridian.Init(ctx, schemas.BifrostConfig{
		Account:            account,
		InitialPoolSize:    s.Config.ClientConfig.InitialPoolSize,
		DropExcessRequests: s.Config.ClientConfig.DropExcessRequests,
		LLMPlugins:         s.Config.GetLoadedLLMPlugins(),
	...
	if err != nil {
		return fmt.Errorf("failed to initialize meridian: %v", err)
	}
	logger.Info("meridian client initialized")
...
func (s *BifrostHTTPServer) Start() error {
	...
	go func() {
		logger.Info("successfully started meridian, serving UI on http://%s", serverAddr)
	...
	case sig := <-sigChan:
		...
		go func() {
			defer close(done)
			logger.Info("shutting down meridian client...")
			s.Client.Shutdown()
			logger.Info("meridian client shutdown completed")
